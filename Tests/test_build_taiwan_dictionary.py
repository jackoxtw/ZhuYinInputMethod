...
s = (Path(__file__).resolve().parents[1] / 'build_and_install.command').read_text('utf-8')
assert 'repair_dictionary_taiwan.py' in s
assert 'libchewing-data' in s
assert 'word.csv' in s
assert 'Resources/dictionary.bin' in s  # offline fallback is already core-repaired
...
assert 'Resources/brand_words.csv' in s
