...
cp Platforms/macOS/App/Info.plist "$CONTENTS/Info.plist"
...
DICT_FOR_BUILD="$PWD/Shared/Resources/dictionary.bin"
...
if [[ -n "$PYTHON3" ]]; then
  ...
  if [[ -f "$TAIWAN_CACHE" ]] && "$PYTHON3" Shared/Tools/repair_dictionary_taiwan.py \
      Shared/Resources/dictionary.bin "$TAIWAN_CACHE" "$TAIWAN_TMP" --require-full; then
...
if [[ -n "$PYTHON3" ]] && "$PYTHON3" Shared/Tools/inject_dictionary_words.py \
    "$DICT_FOR_BUILD" Shared/Resources/brand_words.csv "$BRAND_TMP"; then
...
T2S_FOR_BUILD="$PWD/Shared/Resources/t2s.bin"
...
if [[ -n "$PYTHON3" ]]; then
  ...
  if [[ "$OPENCC_DOWNLOAD_OK" -eq 1 ]] && \
     "$PYTHON3" Shared/Tools/build_tw2s_opencc.py \
...
cp -R Shared/Resources/zh-Hant.lproj "$RES/"
cp -R Shared/Resources/zh_TW.lproj "$RES/"
...
CFLAGS=(-std=c99 -Os -DNDEBUG -D_POSIX_C_SOURCE=200809L -mmacosx-version-min=12.0 -arch "$ARCH" -ffunction-sections -fdata-sections -I"$PWD/Shared/Core")
OBJCFLAGS=(-Os -DNDEBUG -fobjc-arc -mmacosx-version-min=12.0 -arch "$ARCH" -I"$PWD/Shared/Core" -I"$PWD/Platforms/macOS/App")

for f in ZYDictionary ZYEngine ZYComposer ZYLearning ZYConversion; do
  xcrun clang "${CFLAGS[@]}" -c "Shared/Core/$f.c" -o "$OBJ/$f.o"
done
xcrun clang "${OBJCFLAGS[@]}" -c Platforms/macOS/App/main.m -o "$OBJ/main.o"
xcrun clang++ "${OBJCFLAGS[@]}" -std=c++17 -c Platforms/macOS/App/ZYRuntime.mm -o "$OBJ/ZYRuntime.o"
xcrun clang++ "${OBJCFLAGS[@]}" -std=c++17 -c Platforms/macOS/App/ZYCandidatePanel.mm -o "$OBJ/ZYCandidatePanel.o"
xcrun clang++ "${OBJCFLAGS[@]}" -std=c++17 -c Platforms/macOS/App/ZYAccessibilityCaret.mm -o "$OBJ/ZYAccessibilityCaret.o"
xcrun clang++ "${OBJCFLAGS[@]}" -std=c++17 -c Platforms/macOS/App/ZYInputController.mm -o "$OBJ/ZYInputController.o"
...
if [[ "$RELEASE_ONLY" -eq 1 ]]; then
  ...
  PACKAGE_DIR="$PWD/Platforms/macOS/Packaging"
  mkdir -p "$RELEASE_DIR"
  rm -rf "$RELEASE_APP"
